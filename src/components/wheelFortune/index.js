...
class WheelOfFortune extends Component {
  ...
  prepareWheel = () => {
    ...
    this.winner =
      this.props.options.winner ?? Math.floor(Math.random() * this.numberOfSegments);
    ...
    >
      {Array.from({ length: number.length }).map((_, j) => {
        ...
        // Render reward text horizontally
          ...
          return (
            <TSpan y={y - 40} dx={this.fontSize * 0.07} key={`arc-${i}-slice-${j}`}>
              {number.charAt(j)}
            </TSpan>
          );
        }
  ...
  _renderKnob = () => {
    const knobSize = this.props.options.knobSize ? this.props.options.knobSize : 20;
