...
import reportCommentContentStyle from "./ReportCommentContent.style";
import { ReportCommentContentProps } from "./ReportCommentContent.types";
...
const ReportCommentContent = (props: ReportCommentContentProps): JSX.Element => {
  const { pkey = "", timestamp = 0, index, onBackPressedCb } = props || {};
  ...
  } = reportCommentContentStyle(colors);
  ...
  const renderReportConfirmationContent = useCallback(
    () => (
      <ConfirmContent
        ...
        title={t("report_comment_success")}
        icon={
          <Icon
  ...
  const handleSubmitPressed = useCallback(() => {
    setIsLoading(true);
    commentsService
      .reportComment(pkey, index, radioProps[checked].key, reportReason)
      .then(() =>
        dispatch(
          showBottomSheet({
...
export default memo(ReportCommentContent);
